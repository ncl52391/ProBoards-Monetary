#!/usr/bin/env python3
"""Rebuild modules/shop/Monetary Shop.pbp from the shop source files."""
...
def assemble(shop_root: Path, version: str) -> str:
    source = "\n\n".join((shop_root / name).read_text(encoding="utf-8") for name in SOURCE_ORDER)
    return source.replace("{VER}", version)


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--root", type=Path, default=Path(__file__).resolve().parents[1])
    parser.add_argument("--pbp", default="modules/shop/Monetary Shop.pbp")
    parser.add_argument("--output")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()

    root = args.root.resolve()
    shop_root = root / "modules" / "shop"
    ...
    source = assemble(shop_root, version)
    banner = f"/* Monetary System - Shop {version} - Copyright (C) 2015 pixelDepth.net All Rights Reserved. http://support.proboards.com/user/2671 */\n"
